    ...
    if dx == 2 and dy == 2:  # much simpler method
        ...
        for y in range(0, outy):
            ...
            for x in range(0, outx):
                ...
                #
                for ix in range(
                    max(0, math.ceil(sx - dx * filterSize)),
                    min(math.floor(sx + dx * filterSize), inx - 1) + 1,
                ):
                    for iy in range(
                        max(0, math.ceil(sy - dx * filterSize)),
                        min(math.floor(sy + dx * filterSize), iny - 1) + 1,
                    ):
                        pixVal += inbuf[ix, iy]
                        w += 1
                outbuf[x, y] = pixVal / w
    ...
    if dx == 0.5 and dy == 0.5:
        ...
        for y in range(0, outy):
            ...
            for x in range(0, outx):
                ...
                for ix in range(
                    max(0, math.ceil(sx - filterSize)),
                    min(math.floor(sx + filterSize), inx - 1) + 1,
                ):
                    for iy in range(
                        max(0, math.ceil(sy - filterSize)),
                        min(math.floor(sy + filterSize), iny - 1) + 1,
                    ):
                        fx = abs(sx - ix)
                        fy = abs(sy - iy)
...
class BasReliefsettings(bpy.types.PropertyGroup):
    ...
    bit_diameter: bpy.props.FloatProperty(
    ...
    widthmm: bpy.props.IntProperty(
        name="Desired width in mm", default=200, min=5, max=4000
    )
    heightmm: bpy.props.IntProperty(
        name="Desired height in mm", default=150, min=5, max=4000
    )
    thicknessmm: bpy.props.IntProperty(
        name="Thickness in mm", default=15, min=5, max=100
    )

    justifyx: bpy.props.EnumProperty(
        name="X",
        items=[
            ("1", "Left", "", 0),
            ("-0.5", "Centered", "", 1),
    ...
    justifyy: bpy.props.EnumProperty(
        name="Y",
        items=[
            ("1", "Bottom", "", 0),
            ("-0.5", "Centered", "", 2),
    ...
    justifyz: bpy.props.EnumProperty(
        name="Z",
        items=[
            ("-1", "Below 0", "", 0),
            ("-0.5", "Centered", "", 2),
