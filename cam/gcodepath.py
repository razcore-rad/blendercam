...
from . import bridges, chunk, image_utils, pattern, simple, strategy, utils
from .nc import iso
from .opencamlib import opencamlib
...
def exportGcodePath(filename, vertslist, operations):
    ...
    print("EXPORT")
    simple.progress("exporting gcode file")
    t = time.time()
    s = bpy.context.scene
    m = s.cam_machine
    enable_dust = False
    ...
    if m.eval_splitting:  # detect whether splitting will happen
        ...
        if totops > m.split_limit:
            split = True
            filesnum = math.ceil(totops / m.split_limit)
    ...
    for i, o in enumerate(operations):
        ...
        if m.output_tool_definitions:
            c.comment(
                ...
                % (
                    simple.strInUnits(o.cutter_diameter, 4),
                    o.cutter_type,
                    o.cutter_flutes,
                )
            )
        ...
        for vi, vert in enumerate(verts):
            ...
            if vi > 0 and l > 0 and downvector.angle(vect) < plungelimit:
                ...
                v.z >= free_movement_height or vi == 0
            ):  # v.z==last.z==free_movement_height or vi==0
            ...
            if split and processedops > m.split_limit:
                c.rapid(
                    x=last.x * unitcorr,
                    y=last.y * unitcorr,
                    z=free_movement_height * unitcorr,
                )
...
def getPath(context, operation):  # should do all path calculations.
    ...
    if operation.machine_axes == "3":
        ...
        operation.orientation = utils.prepareIndexed(operation)  # TODO RENAME THIS

        getPath3axis(context, operation)  # TODO RENAME THIS

        utils.cleanupIndexed(operation)  # TODO RENAME THIS
...
def getPath3axis(context, operation):
    ...
    if o.strategy == "CUTOUT":
        ...
        if o.strategy == "CARVE":
            ...
            pathSamples.extend(chunk.curveToChunks(ob))
            pathSamples = utils.sortChunks(pathSamples, o)  # sort before sampling
            pathSamples = chunk.chunksRefine(pathSamples, o)
        elif o.strategy == "PENCIL":
            image_utils.prepareArea(o)
            utils.getAmbient(o)
            pathSamples = image_utils.getOffsetImageCavities(o, o.offset_image)
            pathSamples = chunk.limitChunks(pathSamples, o)
            pathSamples = utils.sortChunks(pathSamples, o)  # sort before sampling
            ...
            pathSamples = image_utils.crazyStrokeImageBinary(o, millarea, avoidarea)
            #####
            ...
            pathSamples = pattern.getPathPattern(o)
        ...
        if o.strategy == "PENCIL":  # and bpy.app.debug_value==-3:
            chunks = chunk.chunksCoherency(chunks)
            ...
            for ch in chunks:
                for vi in range(0, len(ch.points)):
                    ch.points[vi] = (
                        ch.points[vi][0],
                        ch.points[vi][1],
                        ch.points[vi][2] - o.carve_depth,
                    )
        if o.use_bridges:
            print(chunks)
            for bridge_chunk in chunks:
                bridges.useBridges(bridge_chunk, o)
        ...
        utils.getAmbient(o)
        chunks = []
        opencamlib.oclGetWaterline(o, chunks)
        chunks = chunk.limitChunks(chunks, o)
        ...
        tw = time.time()
        chunks = []
        simple.progress("retrieving object slices")
        image_utils.prepareArea(o)
        ...
        #######################
        nslices = math.ceil(abs(o.minz / o.slice_detail))
        ...
        for h in range(0, nslices):
            ...
            slicepolys = image_utils.imageToShapely(o, islice, with_border=True)
            ...
            for p in slicepolys:
                ...
                nchunks = chunk.shapelyToChunks(p, z)
                nchunks = chunk.limitChunks(nchunks, o, force=True)
            ...
            if o.waterline_fill:
                ...
                if not lastslice.is_empty or (
                    ...
                    restpoly = restpoly.buffer(
                        -o.dist_between_paths, resolution=o.circle_detail
                    )

                    fillz = z
                    i = 0
                    while not restpoly.is_empty:
                        nchunks = chunk.shapelyToChunks(restpoly, fillz)
                        # project paths TODO: path projection during waterline is not working
                        if o.waterline_project:
                            nchunks = chunk.chunksRefine(nchunks, o)
                            nchunks = utils.sampleChunks(o, nchunks, layers)

                        nchunks = chunk.limitChunks(nchunks, o, force=True)
                        #########################
                        slicechunks.extend(nchunks)
                        pattern.parentChildDist(lastchunks, nchunks, o)
                        lastchunks = nchunks
                ...
                simple.progress("waterline layers ", percent)
                lastslice = poly

            if (
                o.movement_type == "CONVENTIONAL"
                and o.spindle_rotation_direction == "CCW"
            ) or (o.movement_type == "CLIMB" and o.spindle_rotation_direction == "CW"):
                for schunk in slicechunks:
                    schunk.points.reverse()
...
def getPath4axis(context, operation):
    ...
    if o.strategy4axis in ["PARALLELR", "PARALLEL", "HELIX", "CROSS"]:
        path_samples = pattern.getPathPattern4axis(o)
