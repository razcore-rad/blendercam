...
class CAM_OPERATION_PROPERTIES_Panel(CAMButtonsPanel, bpy.types.Panel):
    ...
    def EngagementDisplay(self, operat, layout):
        ...
        if ao.cutter_type == "BALLCONE":
            ...
            layout.label(
                text="Cutter engagement: "
                + str(round(100 * ao.dist_between_paths / ao.ball_radius, 1))
                + "%"
            )
        else:
            ...
            layout.label(
                text="Cutter Engagement: "
                + str(round(100 * ao.dist_between_paths / ao.cutter_diameter, 1))
                + "%"
            )
        ...
        if len(scene.cam_operations) > 0:
            ...
            if ao.valid:
                ...
                if ao.strategy not in ["POCKET", "DRILL", "CURVE", "MEDIAL_AXIS"]:
                    ...
                    layout.operator(
                        "scene.cam_bridges_add", text="Autogenerate bridges"
                    )
